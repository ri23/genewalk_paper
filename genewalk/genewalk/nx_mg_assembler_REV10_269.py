...
class PcNxMgAssembler(NxMgAssembler):
    ...
    def add_pc_edges(self):
        ...
        pc.add_nodes_from(hgnc_symbols)#REV10_269: Add all mapped DEGS = hgnc_symbols to multigraph
...
#         self.graph.remove_nodes_from(nx.isolates(pc_sub))#REV10_269: comment out this line
        ...
        for i, st in enumerate(self.stmts):
            ...
            for a, b in itertools.combinations(agents, 2):
                ...
                self.graph.add_edge(a_node, b_node, key=edge_key,
                                    label=edge_type)
        hgnc_symbols = [g['HGNC_SYMBOL'] for g in self.genes]#REV10_269
        hgnc_ids = [g['HGNC'] for g in self.genes]#REV10_269
        up_ids = [g['UP'] for g in self.genes]#REV10_269
        self.graph.add_nodes_from(hgnc_symbols)#REV10_269 Add all mapped DEGS = hgnc_symbols to multigraph
        gene2hgnc_dict = dict(zip(hgnc_symbols, hgnc_ids))#REV10_269
        nx.set_node_attributes(self.graph, gene2hgnc_dict, 'HGNC')#REV10_269
        gene2up_dict = dict(zip(hgnc_symbols, up_ids))#REV10_269
        nx.set_node_attributes(self.graph, gene2up_dict, 'UP')#REV10_269
