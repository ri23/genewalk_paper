...
class DeepWalk(object):
    ...
    def get_walks(self, workers=1):
        ...
        start_nodes = get_start_nodes(self.graph, self.niter)#REV10_269
        # In case we don't parallelize
        if workers == 1:
            for count, node in enumerate(start_nodes):#REV10_269
                walk = run_single_walk(node, self.graph, self.wl) #REV10_269
                self.walks.append(walk) #REV10_269
                if (count + 1) % 100 == 0:
...
#REV10_269: comment out, no longer needed
