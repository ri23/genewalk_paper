...
class DeepWalk(object):
    ...
    def __init__(self, graph, walk_length=default_walk_length,
                 niter=default_niter,real_graph=True):
        self.graph = graph
        self.walks = []
        self.wl = walk_length
        self.niter = niter
    ...
    def get_walks(self, workers=1):
        ...
        start_nodes = get_start_nodes(self.graph, self.niter, self.real_graph)#REV9GDGO
        # In case we don't parallelize
        if workers == 1:
            for count, node in enumerate(start_nodes):
...
#             start_nodes = get_start_nodes(self.graph, self.niter)#REV9GDGO
...
def get_start_nodes(graph, niter,bool_real_graph):
    start_nodes = []
    if bool_real_graph:#REV9GDGO
        for node in nx.nodes(graph):
            node_dict = graph.nodes[node]
            #REV9GDGO:node is a gene or originating from indra
            if 'UP' in node_dict or ('source' in node_dict and node_dict['source']=='indra'):
                start_nodes += [node for _ in range(niter * len(graph[node]))]
            else:
                #REV9GDGO: node is GO annotation: directly connected to a gene
                if 'GO' in node_dict:
                    subgr = nx.ego_graph(graph,node)
                    if 'GO:annotation' in nx.get_edge_attributes(subgr,'label').values():
                        start_nodes += [node for _ in range(niter * len(graph[node]))]
    else:#randomized graph: get walks over whole graph #REV9GDGO                  
        for node in nx.nodes(graph):
            start_nodes += [node for _ in range(niter * len(graph[node]))]
    return start_nodes
...
def run_walks(graph, **kwargs):
    ...
    **kwargs
    ...
    dw_args = {'walk_length': kwargs.pop('walk_length', default_walk_length),
               ...
               'real_graph':kwargs.pop('real_graph', True)}#REV9GDGO
